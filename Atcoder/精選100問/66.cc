...
//n:=頂点の総数
//計算量:=O(m log{n})
double Kruskal(vector<Edge> &edges,ll n){
...
double dist(vector<double> v1,vector<double> v2){
    double ret=0;
    REP(i,3)ret+=(v1[i]-v2[i])*(v1[i]-v2[i]);
    return sqrt(ret);
}
...
void solve(ll n){
    ...
    //cin.tie(nullptr);
    vector<pair<double,vector<double>>> points(n);
    REP(i,n){
    ...
    REP(i,n){
        REP(j,n){
            edges.PB({i,j,max(double(0),dist(points[i].S,points[j].S)-points[i].F-points[j].F)});
        }
    }
