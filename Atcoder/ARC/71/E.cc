...
signed main(){
    ...
    //cin.tie(nullptr);
    string s,t;ll q;cin>>s>>t>>q;
    ll ls,lt;ls=SIZE(s);lt=SIZE(t);
    BIT xs(ls);BIT xt(lt);
    REP(i,ls){
        if(s[i]=='A'){
            xs.add(i+1,1);
    ...
    REP(_,q){
        ...
        ll sa,sb,ta,tb;
        sa=xs.sum(b)-xs.sum(a-1);
        ta=xt.sum(d)-xt.sum(c-1);
        sb=(b-a+1)-sa;
        tb=(d-c+1)-ta;
        //cout<<sa<<" "<<sb<<" "<<ta<<" "<<tb<<endl;
        //cout<<(sa*2+sb)%3<<" "<<(ta*2+tb)%3<<endl;
        //cout<<(sa+sb*2)%3<<" "<<(ta+tb*2)%3<<endl;
